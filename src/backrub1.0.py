...
def backrub(filepath,n):
    ...
    for line in f:     #finding number of chains and molicule numbers
        temp = line.split(" ")
        if temp[0]== "TER":
    ...
    f.close()
    while True: # finding random segment having information available for all its residues
        flag=1
    ...
    for line in f:
        if ter_count != apply_move_on_chain:#finding the chain we want to update
            temp = line.split("  ")
            if temp[0] == "TER":
                ter_count+=1
        else:
            ...
            if temp[0] == "TER":
                ter_count+=1
                break
            if temp[0] == "ATOM": # applying rotation on selected segment
...
def main():
    ...
    while i < int(sys.argv[3]): #finding 10 ensembles
        ...
        if changeInEnergy <= 0 or random_number > metropolis_criterion:
            ...
            addToEnsemble(newpdb,ensemble_number)
            graph_energies.append(newEnergy)
        print "------------------------------------------------------------------"
    print graph_energies #energies and corresponding change in energy of the ensemble
